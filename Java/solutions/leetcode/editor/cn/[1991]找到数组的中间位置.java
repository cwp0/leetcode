// 
// @Question: [1991]找到数组的中间位置 
// @Author: cwp0
// @CreatedTime: 2024-07-08 10:40:43
// @Description: 
...
class Solution {
    public int findMiddleIndex(int[] nums) {
        ...
        rightPre[n-1] = nums[n-1];
        for (int i = 1; i < n; i++) {
            leftPre[i] = nums[i] + leftPre[i-1];
        }
        for (int i = n-2; i >= 0; i--) {
            rightPre[i] = nums[i] + rightPre[i+1];
        }
        if (rightPre[1] == 0) return 0;
        for (int i = 1; i < n-1; i++) {
            if (rightPre[i+1] == leftPre[i-1]) return i;
        }
        if (leftPre[n-2] == 0) return n-1;
        return -1;
    }
}
