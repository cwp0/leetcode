// 
// @Question: [2981]找出出现至少三次的最长特殊子字符串 I 
// @Author: cwp0
// @CreatedTime: 2024-05-29 10:25:25
// @Description: 
    ...
    public int maximumLength(String s) {
        ...
        for (int i = 0; i < 26; i++) {
            if (chars[i].size() > 0 && chars[i].get(0) - 2 > 0) {
                ans = Math.max(ans, chars[i].get(0) - 2);
            }
            if (chars[i].size() > 1 && chars[i].get(0) - 1 > 0) {
                ans = Math.max(ans, Math.min(chars[i].get(0) - 1, chars[i].get(1)));
            }
            if (chars[i].size() > 2) {
                ans = Math.max(ans, chars[i].get(2));
            }
        }
